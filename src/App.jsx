...
// VITE HMR Çökme Engelleme: Firebase güvenli başlatma
let app;
try {
  app = getApp();
} catch (e) {
...
// --------------------------------------------------------
// 2. TAM KAPSAMLI DİL VE ÇEVİRİ DESTEĞİ
// --------------------------------------------------------
const LANGUAGES = [
...
const tFallback = TRANSLATIONS.en; // Güvenlik katmanı
...
const AVATAR_PRESETS = [
  ...
  "https://api.dicebear.com/7.x/adventurer-neutral/svg?seed=Rocky&backgroundColor=450a0a",
];
...
const CustomAnimations = () => (
  <style dangerouslySetInnerHTML={{__html: `
    ...
    @keyframes flight1 {
      ...
      25%  { transform: translate(65px, 55px) rotate(15deg); } /* Puanın üstüne konma */
...
// Mor -> Mavi -> Kırmızı -> Sarı -> Neon Yeşil
// --------------------------------------------------------
...
// DEVASA ANİMASYON BİLEŞENİ (Sade Neon ve Sinekli Mor)
const MegaScoreVFX = ({ score }) => {
  const s = Number(score);
  if (isNaN(s)) return null;

  // 9.0+ NEON YEŞİL IŞIK VE HALKA
  if (s >= 9.0) {
    return (
  ...
  // < 2.5 KOKUŞMUŞ MOR + SİNEKLER (Saf Işık ve Sinekler)
  if (s < 2.5) {
    return (
...
// MİNİ ANİMASYONLAR (Listelerdeki küçük kutucuklar için)
const MiniVFX = ({ score }) => {
  const s = Number(score);
  if (isNaN(s)) return null;
...
// YATAY KAYDIRMA ŞERİDİ (Tıklanmama & Kaymama Sorunu Kesin Çözüldü)
// Not: Bu bileşen her render'da yok olmaması için artık dışarıda tanımlı.
  ...
  return (
    <div className="mb-10 relative">
      ...
      <div ref={rowRef} className="flex overflow-x-auto gap-4 sm:gap-6 pb-6 pt-2 hide-scrollbar snap-x relative z-10 pointer-events-auto">
        {movies.map((m, i) => {
          // EĞER FİLM TOPLULUK TARAFINDAN OYLANMIŞSA GENEL ORTALAMAYI GÖSTER!
          const globalData = globalMoviesList?.find(g => String(g.id) === String(m.tmdbId));
          
          return (
          <div key={i} className="group cursor-pointer shrink-0 snap-start w-32 sm:w-44" onClick={() => selectMovieToRate(m.tmdbId, m.title)}>
            <div className="relative rounded-2xl overflow-hidden shadow-lg border border-slate-800 mb-3 aspect-[2/3] bg-slate-900 transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_15px_30px_rgba(245,158,11,0.2)]">
              ...
              <div className="absolute inset-0 bg-[#04060C]/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                 <span className="bg-amber-500 text-[#04060C] text-xs sm:text-sm font-black px-4 py-2 rounded-xl flex items-center gap-1 shadow-lg"><Star size={16} className="fill-[#04060C]"/> {t.rateNow || 'Puanla'}</span>
              </div>
              {/* GLOBAL GENEL PUAN ORTALAMASI (Sadece oylanmışsa görünür) */}
...
const getBadges = (ratingCount, t) => {
  ...
  const badges = [];
  if (c >= 1) badges.push({ id: 'b1', name: t.b1Name || 'Mısır Yiyici', icon: <Film size={24}/>, color: 'text-zinc-400 bg-zinc-800/50 border-zinc-700', desc: t.b1Desc });
  if (c >= 10) badges.push({ id: 'b2', name: t.b2Name || 'Sinemasever', icon: <Ticket size={24}/>, color: 'text-sky-400 bg-sky-900/20 border-sky-800/50', desc: t.b2Desc });
  if (c >= 50) badges.push({ id: 'b3', name: t.b3Name || 'Sinefil', icon: <Award size={24}/>, color: 'text-fuchsia-400 bg-fuchsia-900/20 border-fuchsia-800/50 shadow-[0_0_15px_rgba(192,38,211,0.2)]', desc: t.b3Desc });
  if (c >= 100) badges.push({ id: 'b4', name: t.b4Name || 'Altın Bilet', icon: <Medal size={24}/>, color: 'text-amber-400 bg-amber-900/20 border-amber-500/50 shadow-[0_0_20px_rgba(245,158,11,0.3)] animate-pulse', desc: t.b4Desc });
  if (c >= 250) badges.push({ id: 'b5', name: t.b5Name || 'Usta Yönetmen', icon: <Clapperboard size={24}/>, color: 'text-rose-500 bg-rose-900/20 border-rose-500/50 shadow-[0_0_25px_rgba(225,29,72,0.4)]', desc: t.b5Desc });
  if (c >= 500) badges.push({ id: 'b6', name: t.b6Name || 'Sinema Tanrısı', icon: <Crown size={24}/>, color: 'text-emerald-400 bg-emerald-900/20 border-emerald-400/80 shadow-[0_0_30px_rgba(52,211,153,0.6)] animate-bounce', desc: t.b6Desc });
  return badges.reverse();
};

// --------------------------------------------------------
// HATA KALKANI (Error Boundary) - ZORUNLU KORUMA
// --------------------------------------------------------
...
function CineScoreMain() {
  ...
  const t = TRANSLATIONS[lang] || TRANSLATIONS['en'] || tFallback;
  ...
  const getCriteriaData = () => [
    { id: 'c1', name: t.c1 || 'Senaryo', weight: 30, desc: t.c1Desc || '' },
    { id: 'c2', name: t.c2 || 'Oyunculuk', weight: 25, desc: t.c2Desc || '' },
    { id: 'c3', name: t.c3 || 'Sinematografi', weight: 20, desc: t.c3Desc || '' },
    { id: 'c4', name: t.c4 || 'Ses & Müzik', weight: 15, desc: t.c4Desc || '' },
    { id: 'c5', name: t.c5 || 'Kurgu & Akış', weight: 10, desc: t.c5Desc || '' },
  ];
  const criteriaData = getCriteriaData();
  const [scores, setScores] = useState(criteriaData.reduce((acc, c) => ({ ...acc, [c.id]: 5 }), {}));

  // --- SİSTEM DİNLEYİCİLERİ ---
  useEffect(() => {
  ...
  // ANA SAYFA VERİLERİ (Trendler, Kültler, Aksiyon, Dram)
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        // İlk 5 trend film için fragmanları çek (YOUTUBE DEĞİL PLAY İKONU İLE GÖSTERİLİR)
  ...
  // Hero Carousel Auto-Play (Otomatik Dönen Manşet)
  useEffect(() => {
  ...
  // Güvenli Listeler ve Sıralamalar
    ...
    <div className="min-h-screen bg-[#04060C] text-slate-200 font-sans pb-24 relative overflow-x-hidden selection:bg-amber-500/30 selection:text-[#04060C]">
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-[2rem] p-8 relative shadow-2xl">
            ...
            <h3 className="text-2xl font-black text-white mb-6 flex items-center gap-3"><Settings className="text-amber-500"/> {t.editProfile || 'Profili Düzenle'}</h3>
            <form onSubmit={handleUpdateProfile} className="space-y-6">
               <div>
                  <label className="text-xs text-slate-400 font-black uppercase mb-2 block tracking-wider">{t.username || 'Kullanıcı Adı'}</label>
               ...
               <div>
                  <label className="text-xs text-slate-400 font-black uppercase mb-3 block tracking-wider">{t.selectAvatar || 'Avatar Seç'}</label>
               ...
               <button type="submit" className="w-full bg-amber-500 hover:bg-amber-400 text-slate-950 font-black p-4 rounded-xl transition-all shadow-[0_0_15px_rgba(245,158,11,0.3)] hover:scale-[1.02] active:scale-[0.98]">{t.saveChanges || 'Kaydet'}</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2rem] p-8 relative shadow-2xl">
            ...
            <div className="text-center mb-8">
              {/* MORPHING LOGO TASARIMI */}
              <div className="w-20 h-20 rounded-2xl mx-auto flex items-center justify-center mb-4 border-[3px] rotate-3 logo-morph-text" style={{backgroundColor: 'transparent'}}>
                 <Clapperboard size={40} className="fill-current" />
              </div>
              <h2 className="text-4xl font-black tracking-tighter drop-shadow-md logo-morph-text">CINE<span className="text-white">SCORE</span></h2>
            </div>
      ...
      <header className="relative z-40 bg-[#04060C]/80 backdrop-blur-2xl border-b border-slate-800/50 sticky top-0 shadow-sm">
        ...
        <div className="max-w-[90rem] mx-auto px-4 h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 cursor-pointer group shrink-0 logo-morph-text" onClick={handleCloseMovie}>
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center rotate-3 border-[3px] transition-transform duration-500 group-hover:rotate-12 group-hover:scale-105" style={{backgroundColor: 'transparent'}}>
              <Clapperboard size={26} className="fill-current" />
            </div>
            <h1 className="text-3xl font-black tracking-tighter hidden lg:flex items-center drop-shadow-md transition-transform duration-500 group-hover:scale-105" style={{fontFamily: "'Montserrat', sans-serif"}}>
      ...
      <main className="relative z-10 max-w-[90rem] mx-auto px-4 py-6 sm:py-10">
            ...
            {currentFeatured && (
              <div className="relative w-full h-[500px] sm:h-[700px] rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800 group">
                ...
                <div className="absolute bottom-0 left-0 p-8 sm:p-16 max-w-4xl z-10">
                  <div className="flex items-center gap-2 mb-4"><Award className="text-amber-500" size={24}/><span className="text-amber-500 font-black tracking-widest text-sm uppercase drop-shadow">{t.featured || 'Haftanın Öne Çıkanları'}</span></div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <button onClick={() => selectMovieToRate(currentFeatured.tmdbId, currentFeatured.title)} className="px-10 py-5 bg-amber-500 text-[#04060C] hover:bg-amber-400 rounded-full font-black transition-all flex items-center gap-3 shadow-[0_0_30px_rgba(245,158,11,0.5)] text-xl hover:scale-105 active:scale-95">
                      <Star size={24} className="fill-[#04060C]"/> {t.rateNow || 'Puanla'}
                    </button>
                    {currentFeatured.trailerKey && (
                      <a href={`https://www.youtube.com/watch?v=${currentFeatured.trailerKey}`} target="_blank" rel="noreferrer" className="px-10 py-5 bg-slate-900/80 backdrop-blur text-white hover:bg-white hover:text-black border border-slate-700 hover:border-white rounded-full font-black transition-all flex items-center gap-3 shadow-xl text-xl hover:scale-105 active:scale-95 group">
                         <Play size={24} className="fill-current text-white group-hover:text-red-600"/> {t.watchTrailer || 'Fragmanı İzle'}
                      </a>
                    )}
                  </div>
                </div>
        ...
        {activeTab === 'rate' && user && selectedMovie && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in zoom-in-95 duration-300 max-w-7xl mx-auto">
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-8 border border-slate-800 shadow-2xl relative text-center">
                ...
                {/* DEVASA ANA PUAN ÇEMBERİ (Smooth Transition Pürüzsüz Renk Geçișleri) */}
                ...
                <button onClick={saveRating} disabled={isSaving} className="w-full py-5 rounded-full bg-gradient-to-r from-amber-500 to-yellow-400 text-[#04060C] hover:from-amber-400 hover:to-yellow-300 font-black text-xl transition-all shadow-[0_0_20px_rgba(245,158,11,0.4)] flex items-center justify-center gap-2 hover:scale-105 active:scale-95">
                  {isSaving ? <Loader2 className="animate-spin text-black"/> : <Save size={24}/>} 
                  {sortedMyRatings.find(r=>r.id===selectedMovie?.id) ? t.updateRating : t.saveRating}
                </button>
              </div>
            </div>
              ...
              <div className="space-y-12">
                {criteriaData.map((c) => {
                  ...
                  <div key={c.id}>
                    ...
                    {/* Dinamik Renkli Kapsül Slider (Pürüzsüz) */}
        ...
        {activeTab === 'global' && (
          <div className="animate-in fade-in duration-500 space-y-8 max-w-7xl mx-auto">
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-slate-900/80 backdrop-blur-md p-8 sm:p-10 rounded-[2.5rem] border border-slate-800 shadow-xl gap-4">
                <div>
                   <h2 className="text-4xl font-black text-white flex items-center gap-3 drop-shadow-md"><Globe className="text-amber-500" size={36}/> {t.globalRanking || 'Dünya Geneli Sıralama'}</h2>
                ...
                <div className="bg-[#04060C] border border-slate-800 px-8 py-5 rounded-3xl flex items-center gap-5 shadow-inner">
                   ...
                   <span className="text-sm text-slate-500 uppercase font-black tracking-widest leading-tight">{t.registeredMovies || 'Oylanan Filmler'}</span>
                </div>
             </div>
